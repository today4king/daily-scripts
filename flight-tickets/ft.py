...
def print_flight_info(d_date, pay_load):
    ...
    #print(response.status_code)
    file = open("resp_content"+d_date.strftime(day_formate)+".json", "w")
    file.write(response.text)
    file.close()
    ...
    for f in cpi['flightSegments' ]:
        fb+=f['airlineName']
    print(d_date.strftime(day_formate),'\t航班号',cpi['itineraryId' ],'price:\t',cpi['priceList'][0]['adultPrice']+cpi['priceList'][0]['adultTax'],'\t','')
...
if __name__ == "__main__":
    # test_sample_request()
    # print('test pass')
    min_depart_date = datetime.datetime.strptime('2019-08-10', day_formate)
    ...
    while (min_depart_date < max_depart_date):
        ...
        min_depart_date.strftime(day_formate), (min_depart_date + timedelta(days=range_one_days)).strftime(day_formate),
